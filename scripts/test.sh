...
if [[ $TEST_SUITE == "unit" ]]; then
	...
	#echo "command: docker run -d --net=host rlister/etcd"
	#docker run -d --net=host rlister/etcd
	#sleep 10
	#echo "command: SNAP_ETCD_HOST=\"127.0.0.1:4001\" go test -v --tags=integration ./..."
	#if you're using docker-machine, 127.0.0.1 needs to be replaced with your docker-machine ip
	SNAP_ETCD_HOST="http://127.0.0.1:4001" go test -v --tags=integration ./... 
fi