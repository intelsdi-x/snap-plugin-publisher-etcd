...
func TestEtcdPlugin(t *testing.T) {
	config := make(map[string]ctypes.ConfigValue)
	var ip Etcd

	Convey("snap plugin etcd integration testing", t, func() {
		var buf bytes.Buffer

		hostip := os.Getenv("SNAP_ETCD_HOST")
		if len(hostip) == 0 {
			log.Fatal("SNAP_ETCD_HOST is not set")
		}

		config["etcd_host"] = ctypes.ConfigValueStr{Value: hostip}
